...
def admin_list_pending_users(request):
    users = User.objects.filter(verification_status=User.VERIF_PENDING)
    return Response(UserSerializer(users, many=True).data)
...
def upload_landlord_documents(request):
    serializer = LandlordDocumentUploadSerializer(request.user, data=request.data, partial=True)
    if serializer.is_valid():
        serializer.save()
...
def admin_dashboard_analytics(request):
    return Response({
        ...
        "pending_verifications": User.objects.filter(verification_status=User.VERIF_PENDING).count(),
    })
...
def confirm_password_reset(request):
    ...
    if serializer.is_valid():
        ...
        is_valid, message = verify_user_otp(user, serializer.validated_data["otp"])
        if not is_valid:
